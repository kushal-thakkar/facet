    ...
    async def get_columns(self, connection: Connection, table_id: str) -> List[ColumnMetadata]:
        ...
        try:
            ...
            if connection_id in self.metadata and 'columns' in self.metadata[connection_id]:
                ...
                return [
                    column for column in self.metadata[connection_id]['columns']
        ...
        try:
            ...
            if os.path.exists('metadata.json'):
                ...
                for connection_id, connection_metadata in metadata_data.items():
                    self.metadata[connection_id] = {
                        'tables': [TableMetadata(**table) for table in connection_metadata.get('tables', [])],
                        'columns': [ColumnMetadata(**column) for column in connection_metadata.get('columns', [])],
                        'relationships': [RelationshipMetadata(**rel) for rel in connection_metadata.get('relationships', [])]
                    }
        except Exception as e:
        ...
        try:
            ...
            for connection_id, connection_metadata in self.metadata.items():
                metadata_data[connection_id] = {
                    ...
                    'relationships': [rel.dict() for rel in connection_metadata.get('relationships', [])]
                }
