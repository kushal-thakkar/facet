...
class QueryService:
    ...
    async def validate_query(self, connection: Connection, query_model: QueryModel) -> QueryValidationResult:
        """
    ...
    async def explain_query(self, connection: Connection, query_model: QueryModel) -> QueryExplainResult:
        """
        ...
        try:
            ...
            result = QueryExplainResult(
                plan=json.dumps(explanation["plan"], indent=2) if isinstance(explanation["plan"], dict) else explanation["plan"],
    ...
    async def save_to_history(self, connection_id: str, query_model: QueryModel, result: QueryResult) -> None:
        """
    ...
    async def get_history(self, limit: int = 10, offset: int = 0, connection_id: Optional[str] = None) -> List[QueryHistoryEntry]:
        """
        Get query history
        ...
        try:
            ...
            if connection_id:
                filtered_history = [entry for entry in self.query_history if entry.connectionId == connection_id]
            else:
                filtered_history = self.query_history
            
            # Sort by timestamp (newest first)
            sorted_history = sorted(filtered_history, key=lambda entry: entry.timestamp, reverse=True)
