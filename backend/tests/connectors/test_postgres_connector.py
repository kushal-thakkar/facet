"""Unit tests for the PostgreSQL connector implementation."""
...
@pytest.fixture
def mock_connection():
    """Fixture that creates a mock PostgreSQL connection."""
    return Connection(
        id="conn1",
        name="Test PostgreSQL",
        type="postgres",
        config=ConnectionConfig(
            host="localhost", port=5432, database="test", user="user", password="password", ssl=True
        ),
...
def postgres_connector(mock_connection):
    """Fixture that creates a PostgresConnector instance with the mock connection."""
    return PostgresConnector(mock_connection)


class TestPostgresConnector:
    """Test suite for the PostgreSQL connector implementation."""
    ...
    async def test_test_connection(self, mock_connect, postgres_connector):
        """Test the test_connection method works correctly."""
    ...
    async def test_test_connection_error(self, postgres_connector):
        """Test the test_connection method handles errors correctly."""
    ...
    async def test_get_metadata(self, postgres_connector):
        """Test the get_metadata method retrieves database metadata correctly."""
        # Setup mock pool
        mock_pool = MagicMock()
        postgres_connector.pool = mock_pool
    ...
    async def test_execute_query(self, postgres_connector):
        """Test the execute_query method runs SQL queries correctly."""
        ...
        expected_results = [{"id": 1, "name": "Test 1"}, {"id": 2, "name": "Test 2"}]
        expected_columns = [{"name": "id", "type": "varchar"}, {"name": "name", "type": "varchar"}]
