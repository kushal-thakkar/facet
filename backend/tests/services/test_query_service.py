...
def mock_connection():
    return Connection(
        ...
        config=ConnectionConfig(
            host="localhost", port=5432, database="test", user="user", password="password"
        ),
...
class TestQueryService:
    ...
    async def test_save_to_history(self, mock_connection, mock_query_model, mock_query_result):
        ...
        # Save to history
        await query_service.save_to_history(mock_connection.id, mock_query_model, mock_query_result)
