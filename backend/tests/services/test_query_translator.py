...
def simple_query():
    return QueryModel(
        ...
        groupBy=["service"],
        metrics=[Metric(function="count", alias="event_count")],
        sort=[SortOrder(column="event_count", direction="desc")],
        limit=10,
    )
...
def complex_query():
    return QueryModel(
        ...
        timeRange=TimeRange(column="timestamp", range="last_7_days"),
        comparison=Comparison(enabled=True, range="previous_period"),
        sort=[SortOrder(column="request_count", direction="desc")],
        limit=100,
    )
...
class TestSQLTranslator:
    ...
    def test_time_range_translation(self, postgres_translator):
        ...
        query = QueryModel(
            source=QuerySource(connectionId="conn1", table="events"),
            timeRange=TimeRange(column="created_at", range="last_30_days"),
        )
