...
function TimeRangeSelector() {
  ...
  const timeRangeLabel = TIME_RANGE_PRESETS.find(
    preset => preset.id === timeRange.range
  )?.label || 'Select time range';
  
  const comparisonLabel = COMPARISON_OPTIONS.find(
    option => option.id === comparison.range
  )?.label || 'No comparison';
  ...
  const applyTimeRange = () => {
    ...
    actions.updateCurrentExploration({
      timeRange: {
        ...
        customRange: tempTimeSettings.type === 'absolute' ? {
                    ...
                    {tempTimeSettings.type === 'relative' && (
                      <div className="ml-6 flex items-center space-x-2">
                        <select
                          ...
                          onChange={(e) => setTempTimeSettings({
                            ...tempTimeSettings,
                            relative: {
